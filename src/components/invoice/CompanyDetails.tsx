...
export const CompanyDetails = ({
  ...
  const handleFieldUpdate = async (field: string, newValue: string) => {
    try {
      ...
      const {
        error
      } = await supabase.from('Attachment_Info').update({
        [field]: newValue
        ...
        <Collapsible open={companyDetailsOpen} onOpenChange={setCompanyDetailsOpen} className="w-full">
          ...
          <CollapsibleContent>
            <Table>
              <TableBody>
                <TableRow>
                  <TableCell className="font-medium w-48 text-left">Company Street</TableCell>
                  <TableCell className="text-left">
                    <EditableTableCell value={cleanValue(currentInvoice.Invoicing_Comp_Street)} onSave={newValue => handleFieldUpdate('Invoicing_Comp_Street', newValue)} />
                  </TableCell>
                </TableRow>
                <TableRow>
                ...
                <TableRow>
                  ...
                  <TableCell className="text-left">
                    <EditableTableCell value={currentInvoice.Invoicing_Comp_State_Prov} onSave={newValue => handleFieldUpdate('Invoicing_Comp_State_Province', newValue)} />
                  </TableCell>
                </TableRow>
                <TableRow>
